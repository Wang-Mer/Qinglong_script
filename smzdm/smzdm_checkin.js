...
// 引入smzdm bot模块
const { SmzdmBot, requestApi, removeTags, getEnvCookies, wait } = require('../model/bot');
// 引入通知模块
const notify = require('../model/sendNotify');
// 引入crypto-js模块
...
class SmzdmCheckinBot extends SmzdmBot {
  ...
  async run() {
    ...
    // 返回检查登录状态、获取所有奖励信息和额外奖励信息
    return `${msg1}${msg2}${msg3}`;
  }

  // async函数，用于定义异步函数，返回一个Promise对象
async checkin() {
    // 调用requestApi函数，发送一个post请求，请求地址为https://user-api.smzdm.com/checkin，请求头为this.getHeaders()，请求参数为touchstone_event，sk，token，captcha
